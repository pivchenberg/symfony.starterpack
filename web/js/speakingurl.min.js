...
$(document).on('input', '[data-translit-to]', function(e){

	var id = $( this ).data( 'translitTo' )
	var translitToInput =  $('#' + id )
	var slug = null
	if( !translitToInput.length ) return
	slug = getSlug($(this).val());
	translitToInput.val( slug )
	// translitToInput.val( slug ).trigger('input');
	SST.helper.triggerEvent(translitToInput[0], 'input');
});